...
class CNN(nn.Module):
    ...
    """
    def __init__(self, num_classes=100): # Default changed to 100
        super().__init__()
        ...
        self.classifier = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Dropout(p=0.3),
            nn.Linear(256, num_classes) # Outputs 100
        )
    ...
    def get_representation(self):
        """
        Return the mean weights of the final layer. 
        For CIFAR-100, this returns a 100-dimensional semantic vector.
        """
...
class Client:
    def __init__(self, dataset, indices, lr=0.05, batch_size=32):
        ...
        self.model = CNN(num_classes=100).to(device) # Force 100 classes
...
def load_accuracy_results(topologies, seeds, prefix="cifar100"):
    """
...
def convergence_test(results, target_acc=0.35, window=5, threshold=1e-3, eval_interval=5):
    """
    For each topology:
...
if __name__ == "__main__":
    ...
    for topo in topologies_to_run:
        ...
        all_summary_file = f"cifar100_{topo}_all.npy"
        
        missing_seeds = []
        for s in seeds:
            if not os.path.exists(f"cifar100_{topo}_seed{s}.npy"):
        ...
        if len(missing_seeds) == 0:
            ...
            if os.path.exists(all_summary_file):
                ...
                acc_all = [np.load(f"cifar100_{topo}_seed{s}.npy") for s in seeds]
        ...
        acc_all = []
        for s in seeds:
            seed_file = f"cifar100_{topo}_seed{s}.npy"
    ...
    results = load_accuracy_results(topologies_to_run, seeds, prefix="cifar100")
